...
func (e *CoreMemoryEngine) seedCapabilities(ctx context.Context) {
	capabilities := []struct {
		...
		{"cap_bancos", "Eu consulto diretamente PostgreSQL e NietzscheDB (grafo + vetores + cache). Posso fazer queries, criar dados e buscar informacoes em qualquer banco."},
		...
		{"cap_memoria_avancada", "Eu tenho memoria multi-camada: episodica (PostgreSQL), grafo de conhecimento (NietzscheDB graph), busca semantica vetorial (NietzscheDB vector), cache em tempo real (NietzscheDB cache), compressao Krylov (1536D para 64D), consolidacao REM noturna, aprendizado Hebbiano, e repeticao espacada SM-2."},
